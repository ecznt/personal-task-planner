...
- Web Push is an independent browser reminder channel beside in-app Notifications, delivered through VAPID-signed push subscriptions after the MVP shipped; it is a browser push channel, not native, SMS, or email. `post-MVP extension; DEC-106`
...
| DEC-101 | 2026-09-05 | Implement L-022 MVP responsive/accessibility hardening: shared responsive AppShell in a single app layout, header quick-create sheet, tooltip delayed-open motion, safe-area padding, deterministic focus management replacing `autoFocus`, metadata titles, and repo-wide repair of lint/format/type failures. Enable CI on `opencode/develop` for branch push/PR verification. | Implemented, committed, and pushed to `opencode/develop`; CI verified green | Commit `4ccb4d0`; CI runs `33989414398`, `33992498866`; `docs/planning/L-022_HARDENING_PLAN.md` |
| DEC-102 | 2026-09-06 | Repair the L-022 CI evidence and close the security-audit gap: restore shell-era sign-out UI, gate quick-create csrf/areas fetches on dialog open, derive session-boundary login return target from the pathname, refit stale E2E specs to the shell-era UI with hermetic mocks, and resolve the pre-existing `pnpm audit` high findings via reviewed `pnpm-workspace.yaml` dependency overrides. | Implemented, committed, and pushed to `opencode/develop`; CI fully green including `test:e2e` and `test:security` | Commit `608febe`; CI run `33992498866`; `docs/planning/L-022_HARDENING_PLAN.md` |
| DEC-103 | 2026-09-06 | Complete L-023 release readiness: record CI rehearsal evidence (migrate deploy + test:db on a disposable PostgreSQL), formalize the release checklist and 30-day re-audit rollback note, extend the decision log, and run the bounded Graphify incremental update. | Implemented, committed, and pushed to `opencode/develop`; plan marked implemented | Commits `b0518df`, `942bfc6`; CI runs `33992751977`, `33997579621`; `docs/planning/L-023_RELEASE_READINESS_PLAN.md` |
| DEC-104 | 2026-09-07 | Post-MVP vision pass: establish a Linear-inspired design language — complete dark mode with new primitives and accessibility, redone app shell and navigation chrome, cinematic landing and auth pages, and a unified page-header/surface system — and repair API production readiness (worker keep-alive, recurrence creation, localhost-origin allowance). | Implemented, committed, and pushed to `opencode/develop` | Commits `464962d`, `5f1c15c`, `d19ccb7`, `cf3c823`, `bdb5c06`, `70f6ee1`, `1dd3a2c` |
| DEC-105 | 2026-09-13 | Post-MVP productivity slices: lazy global Inbox area plus global Task create and upcoming (`GET /tasks/upcoming`) endpoint; quick capture with Turkish NLP and an Upcoming view; Projects hub with detailed project view; Settings preferences for time zone and notifications; keyboard shortcuts and a command palette; and task filters with URL persistence and a date-state facet. | Implemented, committed, and pushed to `opencode/develop` | Commits `495041f`, `57b35b6`, `1ac6fb0`, `fa39285`, `22666f0`, `953288f`; AGENTS.md branch policy updated in `34db2e9` |
| DEC-106 | 2026-09-14 | Add a Web Push reminder channel as the second notification channel beside in-app Notifications: browser PushSubscription registry (`POST`/`DELETE /push-subscriptions`), independently toggled `pushReminderNotificationsEnabled` preference, VAPID-signed worker delivery at due time, 404/410 subscription cleanup, and 410-removal semantics; no backfill of disabled periods. | Implemented, committed, and pushed to `opencode/develop` | Commits `9bb23d9`, `e17b932`; `docs/api/API_CONTRACT.md` API-D-014, FR-091–FR-093, BR-NOTIF-002 |
| DEC-107 | 2026-09-14 | Implement L-024 Calendar: `GET /tasks/calendar` returning day-bucketed Tasks on planned and due days (≤62 local days, same-day dedupe), month-grid UI with navigation and today highlight, day cells with overflow, and click-to-add using NL quick-add pre-filled with `plannedAt`; excluded week view and drag rescheduling. | Implemented, committed, and pushed to `opencode/develop` | Commits `5208a6e`, `526617e`, `c86165b`, `a899ebf`, `8350f09`; `docs/api/API_CONTRACT.md` API-D-015 |
| DEC-108 | 2026-09-15 | Implement L-025 natural-language Quick Add v2: Todoist-style `#Proje`/`@Etiket` tokens, `p1`–`p3` priority plus Turkish words, dates/times schedule `plannedAt` instead of `dueAt`, live preview in calendar day dialog; unresolved tokens stay verbatim with an inline warning and no auto-creation. | Implemented, committed, and pushed to `opencode/develop` | Commit `5a1c0cb`; `docs/planning/BACKLOG.md` L-025 |
| DEC-109 | 2026-09-15 | Implement L-026 Kanban v2: server-side search/filter toolbar (`q`, `areaId`, `projectId`, `priority`, `labelId`) with enriched `KanbanTaskDto` on global and area boards, URL-persisted filters, and drag-and-drop between columns with optimistic reorder and keyboard fallback; also fixed latent `If-Match` and `taskLabels` query bugs. | Implemented, committed, and pushed to `opencode/develop` | Commits `a1684b5`, `7fa42a6`, `79d86ea`, `152d3ac`, `158be25`; `docs/planning/L-026_KANBAN_V2_PLAN.md` |
| DEC-110 | 2026-09-17 | Post-MVP label and data-quality work: label colors end-to-end (DB drift repair, color field with version, colored chips in all views, hover emphasis, color picker in label management), row-side-panel Task sheet with autosave, immediate label save in task detail, and dummy-user seed plus verify scripts for local testing. | Implemented, committed, and pushed to `opencode/develop` | Commits `70fa394`, `d292c0e`, `f0ec97c`, `266c226`, `38ccefe`, `27a5ad7`, `6f8535b`, `f463c15`, `1a601d4`, `8c02871`, `0fd377f`, `e794632`, `583293c`, `9a8b811`, `c54b489`, `5128ee6`, `e2028ff` |
| DEC-111 | 2026-09-20 | Produce `docs/research/FEATURE_RESEARCH.md` (competitive feature research across Todoist, Things 3, TickTick, MS To Do, Sunsama, Motion, OmniFocus 4, Notion Tasks, Linear) and, from it, decision-complete plan documents for the next slices L-027 (sub-tasks), L-028 (task templates), L-029 (snooze/postpone), L-030 (Markdown notes), and L-031 (Kanban: Area board URL filters + new Project board). | Implemented — documents committed to `opencode/develop` | `docs/research/FEATURE_RESEARCH.md`; `docs/planning/L-027_SUBTASKS_PLAN.md`; `docs/planning/L-028_TASK_TEMPLATES_PLAN.md`; `docs/planning/L-029_SNOOZE_PLAN.md`; `docs/planning/L-030_MARKDOWN_NOTES_PLAN.md`; `docs/planning/L-031_KANBAN_IMPROVEMENTS_PLAN.md` |
| DEC-112 | 2026-09-20 | Implement L-028 Task templates: `TaskTemplate` data model + migration; owned CRUD at `/api/v1/task-templates` (GET cursor list, POST w/ Idempotency-Key, PATCH/DELETE w/ If-Match and version conflict); `POST /:id/apply` reusing `TaskService.createTask` with offset-based `plannedAt`, label name→id resolution (missing silently skipped) and checklist instantiation; `/app/templates` management page, sidebar entry, quick-add "Şablon" tab. | Implemented, committed, and pushed to `opencode/develop` | `docs/planning/L-028_TASK_TEMPLATES_PLAN.md`; `docs/planning/BACKLOG.md` L-028 |
| DEC-113 | 2026-09-21 | Implement L-030 Markdown-supported rich notes on Task description (web render-only, safe subset). Dependency-free tokenizer renderer (`renderMarkdown`) producing React JSX trees — never `dangerouslySetInnerHTML`; `http:`/`https:`-only link sanitization; headings, bold/italic, ordered/unordered lists, GFM task-list checkboxes, blockquotes, fenced code, inline code; `TaskDescription` edit surface with Kaynak/Önizleme toggle in both task detail page and task sheet; XSS-focused unit/component/E2E coverage. | Implemented — app-suite verified locally; committed and pushed to `opencode/develop` | `docs/planning/L-030_MARKDOWN_NOTES_PLAN.md`; `docs/planning/BACKLOG.md` L-030; commit `fb31059` |
| DEC-114 | 2026-09-21 | Implement L-031 Kanban improvements: (1) Area board filters now URL-persisted (`/app/areas/:id?q=&priority=&labelId=&projectId=`), (2) new Project-scoped Kanban — `GET /api/v1/projects/:projectId/kanban` and `POST /api/v1/projects/:projectId/kanban-moves` (If-Match, missing → 428; invalid target → 422 `INVALID_TARGET_STATUS`; stale → 409) with area-shaped `{statuses,columns}` response, (3) shared `StatusKanbanBoard` in `apps/web/src/features/kanban/status-kanban-board.tsx` (area+project single implementation, URL-mode filters, optimistic move), `KanbanToolbar showProject` prop, project-detail Liste/Kanban toggle, project page width `max-w-5xl`. | Implemented — API+web verified locally (typecheck, lint, api unit 172, api contract 157, contract deterministik, web component 243, build, e2e project-kanban 3/3, secret scan); not yet committed/pushed | `docs/planning/L-031_KANBAN_IMPROVEMENTS_PLAN.md`; `docs/planning/BACKLOG.md` L-031 |
...
| `docs/research/FEATURE_RESEARCH.md` | Turkish competitive feature research across 9 task-management products, gap analysis, scoring, and a prioritized shortlist of features worth adopting. | Produced 2026-09-20; input to next-slice decision |
...
| `docs/planning/BACKLOG.md` | Lean implementation backlog, completed baseline, reduced execution rules, slices L-001 through L-026, and post-MVP follow-on work. | Current; revised by DEC-080; L-001 through L-026 implemented; preserves approved MVP scope |
...
| `docs/planning/L-022_HARDENING_PLAN.md` | Implementation plan for L-022 responsive/accessibility hardening. | Implemented; DEC-101/DEC-102 |
| `docs/planning/L-023_RELEASE_READINESS_PLAN.md` | Implementation plan for L-023 release readiness. | Implemented; DEC-103 |
...
| `docs/planning/L-031_KANBAN_IMPROVEMENTS_PLAN.md` | Implementation plan for L-031 Kanban improvements (Area board URL filters + Project board). | Implemented; DEC-111, DEC-114 |
...
Verified BL-121 graph findings:

- The bounded incremental code refresh produced 1,360 nodes, 1,823 edges, and 130 communities.
- One bounded depth-2 query found the intended public-route impact around `HomePage`, `PrivacyPage`, `TermsPage`, `RegistrationForm`, `page.test.tsx`, and the Playwright foundation journey.
...
- Verification: the registered MCP successfully ran one bounded depth-2 `query_graph` against the BL-011 graph; prior `get_node`, `get_community`, `get_neighbors`, and `shortest_path` verification remains valid.
- Verified graph response: 1,240 nodes, 1,667 edges, and 125 communities. The graph contains 98% EXTRACTED, 2% INFERRED, and 0% AMBIGUOUS relationships.
...
| 2026-09-07 | Phase 3 — post-MVP vision pass | Implemented, committed, pushed to `opencode/develop` | Linear-inspired design language: complete dark mode with new primitives and accessibility, redone app shell/navigation, cinematic landing and auth pages, unified page-header/surface system; API production fixes (worker keep-alive, recurrence, localhost origin); DEC-104 |
| 2026-09-13 | Phase 3 — post-MVP productivity slices | Implemented, committed, pushed to `opencode/develop` | Lazy global Inbox + global Task create + Upcoming endpoint, Turkish-NLP quick capture + Upcoming view, Projects hub, Settings preferences, keyboard shortcuts + command palette, URL-persisted task filters; DEC-105 |
| 2026-09-14 | Phase 3 — Web Push reminder channel and L-024 Calendar | Implemented, committed, pushed to `opencode/develop` | VAPID push subscriptions channel with 404/410 cleanup, month-grid Calendar with day quick-add and planned/due bucketing; DEC-106, DEC-107 |
| 2026-09-15 | Phase 3 — L-025 natural-language Quick Add v2 | Implemented, committed, pushed to `opencode/develop` | `#Proje`/`@Etiket`/`p1`–`p3` shorthand, `plannedAt` scheduling, live preview in calendar dialog; DEC-108 |
| 2026-09-15 | Phase 3 — L-026 Kanban v2 | Implemented, committed, pushed to `opencode/develop` | Server-side search/filter toolbar, enriched `KanbanTaskDto`, URL-persisted filters, drag-and-drop with optimistic reorder and keyboard fallback; DEC-109 |
| 2026-09-17 | Phase 3 — post-MVP label and data-quality work | Implemented, committed, pushed to `opencode/develop` | Label colors end-to-end with color picker and chips, row side-panel Task sheet with autosave, dummy-user seed + verify scripts; DEC-110 |

## Current planning stage

Phase 3 Implementation — MVP plus post-MVP product development. EPIC-001; the authentication baseline (BL-007 through BL-011, BL-014, BL-015, BL-121); the lean backlog refactor; L-001 through L-026; and the post-MVP vision and productivity slices (DEC-104 through DEC-110) are all implemented and committed on `opencode/develop`. The MVP is release-ready per the L-023 rehearsal evidence; post-MVP slices continue the same lean cycle.

## Next required action

Continuing post-MVP development — the next slice is currently undefined and awaits a product decision by the user. `docs/planning/BACKLOG.md` lists candidates (email reminders, week view, saved search filters) under "Next slice — TBD". Production deployment remains an explicit user-directed decision.
