...
| `PATCH /users/me` | Session + CSRF + `If-Match` | Current User only. | Exactly one of confirmed account time zone, `inAppReminderNotificationsEnabled`, or `pushReminderNotificationsEnabled`; no email/owner field. Disabling requires explicit acknowledged consequences. | `200` updated User + new `ETag`; timed display changes do not rewrite stored instants, and preference changes do not delete reminders or Notification history. | `401`, `412`, `422`, `428`, `429`. | FR-045–FR-046, FR-091–FR-093, NFR-012, UXF-024, AC-016 |
...
| `POST /push-subscriptions` | Session; `Idem` | Current User only. | Endpoint URL and PWA-256-DH/Auth keys from the browser PushSubscription. | `201` stored subscription id. | `400`, `401`, `429`. | FR-091–FR-093, BR-NOTIF-002 |
| `DELETE /push-subscriptions` | Session | Current User only. | Endpoint URL to remove. | `200` removal acknowledgement. | `400`, `401`, `404`, `429`. | FR-091–FR-093, BR-NOTIF-002 |
...
At the due-time consistency boundary, the internal operation reads the current User preferences. When the in-app preference is enabled it transitions the reminder to Triggered and creates or returns its unique Notification; otherwise it transitions to Suppressed and returns no Notification. When the web push preference is enabled the same operation also delivers a Web Push message to each registered PushSubscription and fingerprints the reminder with `pushDeliveredAt`; subscriptions returning 404/410 are removed. Preference changes never delete existing Notification records or subscriptions, and Suppressed/undelivered reminders are not backfilled after re-enabling.
