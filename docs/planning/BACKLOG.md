...
| L-033 | Today delight trio: Gün Özeti (briefing) + tamamlama kutlaması (confetti + progress ring) + haftalık istatistikler | `docs/planning/L-033_TODAY_DELIGHT_PLAN.md` | Implemented — API `GET /tasks/statistics/weekly` (ternary weekly bucket by `updatedAt` on ACTIVE/COMPLETED), web: `TodayBriefing` (saate göre selamlama + rozetler + en önemli 3 görev), `ProgressRing` (SVG, gradient), `Celebration` (kütüphanesiz WAAPI confetti, reduced-motion dahil), `WeeklyStats` (7 günlük bar chart, `.next` build). App-suite verified (api typecheck/lint/unit 175/api 160, contract deterministic, web typecheck/lint/component 247, build, e2e foundation 9/9) |

Explicit user request on 2026-09-20: plan L-027 through L-030 plus Kanban (Area improvement + Project board). All five research "Adopt" features from `docs/research/FEATURE_RESEARCH.md` are now implemented as L-027 (F-01), L-028 (F-05), L-029 (F-02), L-030 (F-08), and L-032 (F-04). On 2026-09-22 the user chose a "daily delight" trio (L-033) instead of the remaining research candidates: Gün Özeti briefing, tamamlama kutlaması, haftalık istatistikler. Remaining research candidates wait for the user to pick the next slice: F-03 (saved filters/smart lists), F-06 (Today "Bu akşam" + end-of-day rollover), F-07 (weekly goals + review).
