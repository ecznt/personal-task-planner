...
**Definition of done:** BL-001 through BL-006 are accepted; SPIKE-001 records a supported generator or an approved follow-up decision; docs/run commands match CI; no bypassed required check or untracked generated contract remains.
...
| BL-008 | A registrant can verify email with a single-use action. | A valid action activates the identity once; expired/invalid actions are safe and actionable; replay cannot create a second identity or session. | API, DB, E2E |
