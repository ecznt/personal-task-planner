...
export class TrashController {
  ...
  @ApiOperation({ operationId: 'listTrash', summary: 'List retained Trash' })
  ...
  async list(@Req() request: Request, @Res() response: Response, @Query() query: unknown): Promise<void> {
    ...
    const result = await this.lifecycle.list(userId, 'TRASHED', input.cursor, input.limit);
  ...
  async detail(
    ...
    response.json({
      data: {
        ...
        trashedAt: result.data.node.trashedAt,
        purgeAfter: result.data.node.purgeAfter,
  ...
  async permanentDelete(
    ...
    @Param('resourceType') resourceType: string,
    @Param('id') id: string,
    @Body() body: { confirmPermanentDelete?: boolean },
    @Headers('if-match') ifMatch?: string,
    @Headers('idempotency-key') idempotencyKey?: string,
  ): Promise<void> {
    const userId = await resolveUserId(request, this.accounts, this.security);
